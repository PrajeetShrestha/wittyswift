...
class WebRTCClient: NSObject, RTCPeerConnectionDelegate, RTCVideoViewDelegate, RTCDataChannelDelegate {
    ...
    func setup(videoTrack: Bool, audioTrack: Bool, dataChannel: Bool, customFrameCapturer: Bool, videoSize: CGSize = CGSize(width: 1280, height: 720), fps: Int = 30){
        ...
        if self.channels.video {
            startCaptureLocalVideo(cameraPositon: self.cameraDevicePosition, videoWidth: Int(videoSize.width), videoHeight: Int(videoSize.height), videoFps: fps)
    ...
    func switchCameraPosition(){
        if let capturer = self.videoCapturer as? RTCCameraVideoCapturer {
            capturer.stopCapture { [self] in
                ...
                self.startCaptureLocalVideo(cameraPositon: self.cameraDevicePosition, videoWidth: Int(self.videoSize.width), videoHeight: Int(self.videoSize.height), videoFps: self.fps)
            }
        } else if let capturer = self.videoCapturer as? RTCCustomFrameCapturer {
    ...
    // MARK: Connect
    func transcribeConnect(onSuccess: @escaping (RTCSessionDescription) -> Void){
        self.peerConnection = setupPeerConnection()
        self.peerConnection!.delegate = self
        let rt = RTCRtpTransceiverInit()
        rt.direction = .recvOnly
        if self.channels.video {
            self.peerConnection!.addTransceiver(of: .video, init:rt)
        }
        if self.channels.audio {
            self.peerConnection!.addTransceiver(of: .audio, init:rt)
        }
        if self.channels.datachannel {
            self.dataChannel = self.setupDataChannel()
            self.dataChannel?.delegate = self
...
//        rtcConf.sdpSemantics = .unifiedPlan
        ...
        //        pc.setBweMinBitrateBps(nil, currentBitrateBps: nil, maxBitrateBps: 2000)
        return pc
    }
